...
function ShareOptionButton({ icon, label, onClick, href }) {
  const className =
    'flex w-full items-center gap-3 rounded-xl border border-slate-200 bg-white px-4 py-3 text-left text-sm font-semibold text-slate-800 transition-colors hover:border-[#04AA6D] hover:bg-green-50 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100 dark:hover:border-green-600 dark:hover:bg-green-950/40'
  ...
  if (href) {
    return (
      <a
        ...
        onClick={onClick}
      >
        <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-slate-100 text-slate-700 dark:bg-slate-700 dark:text-slate-200">
          {icon}
        </span>
        {label}
      </a>
 ...
 */
export function ShareModal({ open, onClose, lessonTitle, url: urlProp }) {
  const panelRef = useFocusTrap(open)
  const [copied, setCopied] = useState(false)
  const url = urlProp || getLessonShareUrl()
        ...
        >
          ...
          >
            ...
            <div className="space-y-3 px-5 py-5">
              ...
              <p className="break-all rounded-lg bg-slate-50 px-3 py-2 font-mono text-xs text-slate-600 dark:bg-slate-800 dark:text-slate-400">
                {url}
              </p>
              ...
              <ShareOptionButton
                ...
                href={buildTwitterShareUrl(lessonTitle, url)}
              />

              <ShareOptionButton
                icon={<WhatsAppIcon className="h-5 w-5 text-[#25D366]" />}
                label="Share on WhatsApp"
                href={buildWhatsAppShareUrl(lessonTitle, url)}
              />
