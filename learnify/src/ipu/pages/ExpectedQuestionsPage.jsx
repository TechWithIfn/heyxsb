...
export default function ExpectedQuestionsPage() {
  ...
  const grouped = useMemo(() => {
    ...
    return topics.map((t) => ({ topic: t, examQuestions: (t.examQuestions || t.content?.examQuestions || []) }))
  }, [subject])
  ...
  const filtered = grouped.filter((g) => {
    ...
    return (
      ...
      (g.examQuestions || []).some((eq) => String(eq).toLowerCase().includes(q))
    )
  })
  ...
  const downloadAll = () => {
    const lines = []
    grouped.forEach((g) => {
      if ((g.examQuestions || []).length === 0) return
      lines.push(`# ${g.topic.topicTitle || g.topic.title}`)
      lines.push(...g.examQuestions.map((q, i) => `${i + 1}. ${q}`))
      lines.push('\n')
    })
  ...
  return (
    <main className="mx-auto max-w-4xl px-4 py-10">
      ...
      <label className="relative mt-6 block">
        ...
        <input value={filter} onChange={(e) => setFilter(e.target.value)} placeholder="Search questions or topics..." className="w-full rounded-2xl border border-slate-200 py-3 pl-12 pr-4" />
      </label>
        ...
        {filtered.map((g) => (
          <section key={g.topic.topicId || g.topic.id} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
            ...
            <div className="mt-3 space-y-2">
              {(g.examQuestions || []).length === 0 ? (
                <p className="text-sm text-slate-500">No expected questions listed for this topic.</p>
              ) : (
                <ol className="list-decimal pl-5">
                  {(g.examQuestions || []).map((q, i) => (
